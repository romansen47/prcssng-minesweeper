...
public class Gui extends processing.template.Gui implements IGui{
	 ...
	 */
	public Gui(Game match) {this.setMatch(match);}
	...
	public void zoom(double e) {
		if (zoom+e*0.2>1) {
			...
			if (Constants.xDefault-(int)(e*0.2*mouseX)<=0 &&
					(Constants.xDefault-(int)(e*0.2*mouseX)+Match.getMatch()[0].length*Constants.CellSize*zoom>
				...
				if (Constants.xDefault-(int)(e*0.2*mouseX)>0) {
					...
					Constants.xDefault+=-(int)(e*0.2*mouseX)+Match.getMatch()[0].length*Constants.CellSize*zoom-Match.getMatch()[0].length*Constants.CellSize;
				}
			}
			if (Constants.yDefault-(int)(e*0.2*mouseY)<=0 &&
					(Constants.yDefault-(int)(e*0.2*mouseY)+Match.getMatch().length*Constants.CellSize*zoom>
				...
				if (Constants.yDefault-(int)(e*0.2*mouseY)>0) {
					...
					Constants.yDefault+=-(int)(e*0.2*mouseY)+Match.getMatch().length*Constants.CellSize*zoom-Match.getMatch().length*Constants.CellSize;
				}
			}
		}
	}
	...
	public void setup() {
		...
		Constants.CellSize=(int)(Math.min(Constants.Horizontal,Constants.Vertical)/SIZE);
		...
		Constants.Stuck=(int)(0.2*(float)Constants.CellSize);
	...
	public void genMinefieldFromImage(){
		...
		for (int i=0;i<Match.getMatch().length;i++) {
			for (int j=0;j<Match.getMatch()[0].length;j++) {
				Color pixCol=new Color(Constants.minefield.get(
						(int)(j*Constants.minefield.width/(SIZE*Constants.ratio)),
						(int)(i*Constants.minefield.height/SIZE)));
				int r=pixCol.getRed();
				...
				*/
				...
				else {Match.getMatch()[i][j].setIsNotABomb();}
			}
		}
	}
	...
	public void draw() {
		...
		if ((mousePressed) && (mouseButton==RIGHT)) {
			if ((Constants.xDefault+mouseX-pressedPos[1]<=0)
					&&
				(Constants.xDefault+mouseX-pressedPos[1]+Match.getMatch()[0].length*Constants.CellSize*zoom>=Match.getMatch()[0].length*Constants.CellSize)
					) {
			...
			if ((Constants.yDefault+mouseY-pressedPos[0]<=0) 
					&&
				(Constants.yDefault+Match.getMatch().length*Constants.CellSize*zoom+mouseY-pressedPos[0]>=Match.getMatch().length*Constants.CellSize))
					{
		...
		if (keyPressed) {
			switch (key) {
				case 'r': Match.reset();zoom=1;break;
				case 'n': Match.renew();break;
				case 'q': exit();break;
				case '+': zoom=zoom+(float)0.2;break;
				case '-': zoom=zoom-(float)0.2;break;
			}	
		}
		...
		if (Constants.restart){
			...
			if (Match.isRunning()==false){
				...
				if (Case==1 && mouseButton==LEFT) {
					...
					fill(255,0,0);
					textSize(30);
					text("You "+Constants.success+"!",(int)(0.2*Constants.Horizontal),(int)(0.5*Constants.Vertical));
				...
				case 1:
					if (mouseButton==LEFT) {
						Constants.success=Match.leftClicked(this,getYIndexFromCoordinates(mouseY)-1,getXIndexFromCoordinates(mouseX)-1);
					}
					if (mouseButton==RIGHT){
						Match.getMatch()[getYIndexFromCoordinates(mouseY)-1][getXIndexFromCoordinates(mouseX)-1].
						setMarkedAsBomb(!(Match.getMatch()[getYIndexFromCoordinates(mouseY)-1][getXIndexFromCoordinates(mouseX)-1].
								isMarkedAsBomb()));
					}
					break;
				case 2:
