...
public interface IGui{
	 ...
	 */
	public static void setSurrBombs(int m,int n,Game Mines) {
		int tmpSurrBombs=0;
		for (int i=-1;i<2;i++) {
			for (int j=-1;j<2;j++) {
				if ( (!((i==0) && (j==0))) &&
					 (m+i>=0) && ((m+i)<Mines.getMatch().length) &&
					 (n+j>=0) && ((n+j)<Mines.getMatch()[0].length) &&
					 Mines.getMatch()[m+i][n+j].getIsBomb() ) {
	 ...
	 * sets bombs randomly
	 */
	public static void setRandomBombs(Game Mines) {
		for (int i=0;i<Mines.getMatch().length;i++) {
			for (int j=0;j<Mines.getMatch()[0].length;j++) { 
